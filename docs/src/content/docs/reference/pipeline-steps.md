...
- When the exact submitted head is a deliberate merge whose second parent is still the direct private gate-mirror head, the default rebase strategy finishes its rewrite with a topology-only merge: the rebased head is the first parent, the submitted merge is the second, and the tree must remain byte-identical to the rebased result. Before creating that bridge, Rebase applies the private-mirror reconciliation proof to ensure the rebased head retained the mirror's content. Missing content fails Rebase; if the private mirror no longer matches that exact parent, no bridge is synthesized and the normal publication-time reconciliation guard still decides publication
