...
Use `pi` with a provider-qualified model such as `xai/grok-4.6`, or `cursor` with a catalog model such as `cursor-grok-4.6-high`, when the repository has `disable_project_settings: true`. Pi uses its verified `--no-context-files` isolation. Cursor's reviewer-only route uses a disposable committed snapshot with Cursor-discovered project-instruction surfaces removed and isolated Cursor state; it is read-only, so Cursor cannot be the primary or fixer under this safeguard. Selecting native `grok` or an unverified `acp:cursor` route is still refused.
