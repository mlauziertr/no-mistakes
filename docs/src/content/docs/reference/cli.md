...
Reviewer models must be slash-separated identifiers made from letters, digits, `.`, `_`, and `-`; empty or relative-path components, leading hyphens, URLs, and credential-shaped values are rejected before launch or persistence. The selected reviewer appears in structured AXI status and daemon run/receipt responses.
